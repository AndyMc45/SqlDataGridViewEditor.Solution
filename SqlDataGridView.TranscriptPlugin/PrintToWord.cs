                    ...
                    using (MemoryStream stream = new MemoryStream())
                    {
                        ...
                        using (WordprocessingDocument wordDoc = WordprocessingDocument.Open(stream, true))
                        {
                            ...
                            foreach (DataRow transDR in transcriptDT.Rows)
                            {
                                ...
                                int requirementNameID = Int32.Parse(dataHelper.getColumnValueinDR(transDR, "requirementNameID"));
                                List<string> reqNameColNames = new List<string> { "reqName", "eReqName" };
                                Dictionary<string, string> reqNameColValues = TranscriptHelper.GetPkRowColumnValues(
                                        TableName.requirementName, requirementNameID, reqNameColNames, ref sbErrors);
                                string reqName = reqNameColValues["reqName"];
                                string eReqName = reqNameColValues["eReqName"];
                                ...
                                if (printJob == PrintJob.printEnglishTranscript)
                                {
                                    if (!String.IsNullOrEmpty(eReqName)) { reqName = eReqName; }
                                ...
                                if (reqName != department)
                                {
                                    InsertTextInTable(table, currentRow, 6, reqName, JustificationValues.Left);
                                }
                            ...
                            // Find Requirement table and fill it
                            if (PrintToWord.studentReqDT != null)
                            {
                                ...
                                foreach (DataRow reqDR in PrintToWord.studentReqDT.Rows)
                                {
                                    ...
                                    InsertTextInTable(table, currentRow, 1, reqName, JustificationValues.Left);
